...
export class PublicSearchPageComponent implements OnInit {
  ...
  public meanConcept: number[] = [];
  public meanTested: number[] = [];
  ...
  async fetchMaterial(){
    ...
    let getResponse = await this.crudService.getAllBNEMaterial();
    if(getResponse){
      this.material = getResponse;
      this.materialSorted = this.material;
    }else{
      this.dialog.open(GenericDialogComponent,{data:{message:"Fehler beim Laden des Materials."}})
    }
  }
